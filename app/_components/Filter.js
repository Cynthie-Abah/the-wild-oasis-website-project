...
const activeFilter = searchParams.get('capacity') ?? 'all'
  ...
  return (
    <div className="border border-primary-800 flex">
        ...
        <Button filter={'large'} handleFilter={handleFilter} activeFilter={activeFilter} >8-12 guest</Button>
    </div>
  )
}
...
return  <button onClick={()=> handleFilter(filter)} className={`${activeFilter === filter ? 'bg-primary-700 text-primary-50' : ''} px-5 py-2 hover:bg-primary-700 light`}>{children}</button>
}